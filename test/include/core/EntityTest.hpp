...
TUT_TEST EntityTest::test<2>() {
    ...
    ensure_equals((entity->get<MockComponent>()).get(), &component);
}
...
TUT_TEST EntityTest::test<3>() {
    ...
    shared_ptr<MockComponent> c1;
    shared_ptr<MockComponent2> c2;

    entity->add(c1);
    entity->add(c2);

    ensure_equals(entity->get<MockComponent>().get(), &c1);
    ensure_equals(entity->get<MockComponent2>().get(), &c2);
}
...
TUT_TEST EntityTest::test<4>() {
    ...
    shared_ptr<Entity>entity;
    shared_ptr<MockComponent> c1, c2;
    entity->add(c1);
    entity->add(c2);

    ensure_equals(entity->get<MockComponent>().get(), &c2);
}
...
TUT_TEST EntityTest::test<5>() {
    ...
    shared_ptr<Entity>entity;
    shared_ptr<MockComponent> c1;
    shared_ptr<MockComponentExtended> c2;

    entity->add(c1);
    entity->add<MockComponentExtended>(c2);

    ensure_equals(entity->get<MockComponent>().get(), c1);
    ensure_equals(entity->get<MockComponentExtended>().get(), c2);
}
...
TUT_TEST EntityTest::test<8>() {
    ...
    ensure_not(find(all.begin(), all.end(), shared_ptr<MockComponent>(&c1)) == all.end());
    ensure_not(find(all.begin(), all.end(), shared_ptr<MockComponent2>(&c2)) == all.end());
}
...
TUT_TEST EntityTest::test<9>() {
    ...
    shared_ptr<Entity>entity;
    shared_ptr<MockComponent2> component;
...
TUT_TEST EntityTest::test<18>() {
    ...
    shared_ptr<Entity> entity("anything");
    entity->name("other");
